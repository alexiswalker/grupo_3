...
```{r include=FALSE}
ipc_prophet_model <- prophet(ipc_train)
```
...
```{r}
ipc_future <- make_future_dataframe(ipc_prophet_model, periods = 9, freq = "month", include_history = FALSE)
...
```{r include=FALSE}
is_prophet_model <- prophet(is_train)
```
...
```{r}
is_future <- make_future_dataframe(is_prophet_model, periods = 9, freq = "month", include_history = FALSE)
...
```{r include=FALSE}
bm_prophet_model <- prophet(bm_train)
```
...
```{r}
bm_future <- make_future_dataframe(bm_prophet_model, periods = 9, freq = "month", include_history = FALSE)
