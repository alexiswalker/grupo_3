...
# original_test_ipc <- tail(ts_ipc, n=length(test_ipc) + ipc_ndiffs)
# original_test_bm <- tail(ts_bm, n=length(test_bm) + bm_ndiffs)
# original_test_is <- tail(ts_is, n=length(test_bm) + is_ndiffs)
...
#accuracy(ipc_bm_model_forecast$forecast$train_ipc$mean, test_ipc)
ipc_var_accuracy <-accuracy(ipc_is_model_forecast$forecast$train_ipc$mean, x = test_ipc)

is_var_accuracy <- accuracy(ipc_is_model_forecast$forecast$train_is$mean, x = test_is)
#accuracy(is_bm_model_forecast$forecast$train_is$mean, test_is)

bm_var_accuracy <- accuracy(ipc_bm_model_forecast$forecast$train_bm$mean, x = test_bm)
#accuracy(is_bm_model_forecast$forecast$train_bm$mean, test_bm)

#accuracy(x = tail(original_test_ipc, n=length(test_ipc)), tail(diff_inv(ipc_bm_model_forecast$forecast$train_ipc$mean, original_test_ipc, ipc_ndiffs), n=length(test_ipc)))
#ipc_var_accuracy <- accuracy(x = tail(original_test_ipc, n=length(test_is)), tail(diff_inv(ipc_is_model_forecast$forecast$train_ipc$mean, original_test_ipc, ipc_ndiffs), n=length(test_ipc)))

#is_var_accuracy <- accuracy(x = tail(original_test_is, n=length(test_is)), tail(diff_inv(ipc_is_model_forecast$forecast$train_is$mean, original_test_is, is_ndiffs), n=length(test_is)))
#accuracy(tail(original_test_is, n=length(test_is)), tail(diff_inv(is_bm_model_forecast$forecast$train_is$mean, original_test_is, is_ndiffs), n=length(test_is)))

#accuracy(tail(original_test_bm, n=length(test_bm)), tail(diff_inv(ipc_bm_model_forecast$forecast$train_bm$mean, original_test_bm, bm_ndiffs), n=length(test_bm)))
#bm_var_accuracy <- accuracy(x = tail(original_test_bm, n=length(test_bm)), tail(diff_inv(is_bm_model_forecast$forecast$train_bm$mean, original_test_bm, bm_ndiffs), n=length(test_bm)))
```
...
metrics_ipc_var <- ipc_var_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(ipc_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR", .before = "ME") 

rbind(metrics_ipc_pp, metrics_ipc_var) %>% 
  kable(booktabs = T, caption = "Métricas de los modelos de IPC", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
TODO: Conclusion de las metricas?
...
```{r fig.cap="Predicciones de IPC", fig.height=5, fig.width=8}
...
Resulta interesante mencionar que los intervalos de confianza de las predicciones son mas acotado que el obtenido con los modelos VAR analizados anteriormente.

```{r ipc-pp-componentes, fig.cap="Componentes del modelo IPC"}
prophet_plot_components(ipc_prophet_model, ipc_prophet_forecast)
```

Ambas elecciones afectaron la serie de forma positiva, aumentando entre 1,5% y 3% por sobre la tendencia. Del otro lado, el inicio del aislamiento por COVID afecta la serie disminiyendo un 1%.
...
metrics_is_var <- is_var_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(is_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR", .before = "ME") 

rbind(metrics_is_pp, metrics_is_var) %>% 
  kable(booktabs = T, caption = "Métricas de los modelos de Indice de Salarios", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
```{r fig.cap="Predicciones de Indice Salarios", fig.height=5, fig.width=8}
...
```
Ambas elecciones afectaron la serie de forma positiva, aumentando aproximadamente en un 2% por sobre la tendencia. El inicio del aislamiento por COVID afecta la serie aumentandola un 3,5%.

Un resultado esperable es que la serie presenta una estacionalidad positiva en diciembre, aumentando el valor de la misma para mediados de mes.
...
metrics_bm_var <- bm_var_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(bm_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR", .before = "ME") 

rbind(metrics_bm_pp, metrics_bm_var) %>% 
  kable(booktabs = T, caption = "Métricas de los modelos de Base Monetaria", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
```{r fig.cap="Predicciones de Base Monetaria", fig.height=5, fig.width=8}
...
Ambas elecciones afectaron la serie de forma negativa, disminuyendo aproximadamente en un 25% la Base Monetaria Acumulada sobre la tendencia. El inicio del aislamiento por COVID afecta la serie aumentandola hasta un 20% sobre la tendencia.

A mitad de anio la base monetaria presenta su pico maximo de estacionalidad positiva, coincidiendo con los aguinaldos.
