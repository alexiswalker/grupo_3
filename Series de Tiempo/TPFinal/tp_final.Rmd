...
e <- c(normality_test$jb.mul$JB$statistic, normality_test$jb.mul$Skewness$statistic, normality_test$jb.mul$Kurtosis$statistic)
p <- c(normality_test$jb.mul$JB$parameter, normality_test$jb.mul$Skewness$parameter, normality_test$jb.mul$Kurtosis$parameter)
pv <- c(normality_test$jb.mul$JB$p.value, normality_test$jb.mul$Skewness$p.value, normality_test$jb.mul$Kurtosis$p.value)
...
normality_test %>%
  kable(booktabs = T, caption = "Residuos: Normalidad", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
shapiro_test_base_monetaria <- shapiro.test(resid[, 1])
shapiro_test_base_monetaria_sin_37 <- shapiro.test(resid[-c(37), 1])
shapiro_test_ipc <- shapiro.test(resid[, 2])
```

Los resultados resumidos en la Tabla \@ref(tab:jb) no muestran residuos distribuidos normalmente. Si se analizan los generados por cada ecuación por separado, la prueba de Shapiro-Wilk (p-value igual a `r shapiro_test_base_monetaria$p.value`) para el índice de precios señala normalidad, mientras que para la base monetaria no (p-value igual a `r shapiro_test_ipc$p.value`).

De revisar la Figura \@ref(fig:ipc-vs-is) se observa un valor atípico para el mes de marzo de 2020, eliminando esta lectura se obtiene un p-value igual a `r shapiro_test_base_monetaria_sin_37$p.value` observando normalidad.
...
```{r}
cause_base_monetaria <- causality(var_model, cause = "dif_ts_base_monetaria_star")

base_monetaria_granger <- data.frame(cause_base_monetaria$Granger$method,
                                     cause_base_monetaria$Granger$p.value)

colnames(base_monetaria_granger) <- c("Método", "P-Value")

base_monetaria_granger %>%
  kable(booktabs = T, caption = "Causalidad de Grange: Base Monetaria", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```

```{r}
cause_ipc <- causality(var_model, cause = "dif_ts_indice_precios_consumidor")
...
```{r}
irf_base_monetaria <- irf(var_model, 
                          impulse = "dif_ts_base_monetaria_star", 
                          response = "dif_ts_indice_precios_consumidor",  
                          n.ahead = 11)

irf_base_monetaria_data <- data.frame(irf_base_monetaria$irf$dif_ts_base_monetaria_star[ ,1],
                                      irf_base_monetaria$Lower$dif_ts_base_monetaria_star[ ,1],
                                      irf_base_monetaria$Upper$dif_ts_base_monetaria_star[ ,1], 
                                      0:11)
colnames(irf_base_monetaria_data) <- c('irf', 'lower', 'upper', 'lags')

irf_ipc <- irf(var_model,
               impulse = "dif_ts_indice_precios_consumidor",
               response = "dif_ts_base_monetaria_star",
               n.ahead = 11)

irf_ipc_data <- data.frame(irf_ipc$irf$dif_ts_indice_precios_consumidor[ ,1],
                                      irf_ipc$Lower$dif_ts_indice_precios_consumidor[ ,1],
                                      irf_ipc$Upper$dif_ts_indice_precios_consumidor[ ,1], 
                                      0:11)
...
irf_base_monetaria_data %>%  ggplot(aes(lags, irf)) +
...
irf_ipc_data %>%  ggplot(aes(lags, irf)) +
            geom_line(aes(y = upper), colour = 'lightblue2') +
            geom_line(aes(y = lower), colour = 'lightblue')+
            geom_line(aes(y = irf))+
            geom_ribbon(aes(x=lags, ymax=upper, ymin=lower), fill="lightblue", alpha=.1) +
