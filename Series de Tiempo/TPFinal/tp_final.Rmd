...
# train_data_ipc <- window(dif_ts_ipc, end = c(2021, 04))
# test_data_ipc <- window(dif_ts_ipc, start = c(2021, 05))
# 
# train_data_bm <- window(dif_ts_bm, end = c(2021, 04))
# test_data_bm <- window(dif_ts_bm, start = c(2021, 05))
# 
# var_train_data <- cbind(train_data_ipc, train_data_bm)
# var_test_data <- cbind(test_data_ipc, test_data_bm)

var_data <- cbind(dif_ts_ipc, dif_ts_bm)
```
...
```{r}
reverse_diff <- function(stationary_data, non_stationary_last_value){
  if(nrow(stationary_data)) {
    f <- head(stationary_data, 1) + non_stationary_last_value
    r <- tail(stationary_data, -1)
    return(rbind(f, reverse_diff(r, f$`Point Forecast`)))
  }
}
```
...
```{r}
last_value <- as.double(tail(diff(ts_ipc, lag = 1, differences = 1), 1))
forecast_ipc <- reverse_diff(as.data.frame(var_forecast$forecast$dif_ts_ipc), last_value)
last_value <- as.double(tail(ts_ipc, 1))
forecast_ipc <- reverse_diff(forecast_ipc, last_value)

forecast_bm <- reverse_diff(as.data.frame(var_forecast$forecast$dif_ts_bm), as.double(tail(ts_bm, 1)))
```


```{r}
var_forecast$forecast$dif_ts_ipc$mean = ts(forecast_ipc$`Point Forecast`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_ipc$lower[, 1] = ts(forecast_ipc$`Lo 80`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_ipc$lower[, 2] = ts(forecast_ipc$`Lo 95`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_ipc$upper[, 1] = ts(forecast_ipc$`Hi 80`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_ipc$upper[, 2] = ts(forecast_ipc$`Hi 95`, frequency = 12, start = c(2021, 8))

var_forecast$forecast$dif_ts_bm$mean = ts(forecast_bm$`Point Forecast`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_bm$lower[, 1] = ts(forecast_bm$`Lo 80`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_bm$lower[, 2] = ts(forecast_bm$`Lo 95`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_bm$upper[, 1] = ts(forecast_bm$`Hi 80`, frequency = 12, start = c(2021, 8))
var_forecast$forecast$dif_ts_bm$upper[, 2] = ts(forecast_bm$`Hi 95`, frequency = 12, start = c(2021, 8))
```
...
reverse_diff(as.data.frame(var_forecast$forecast$dif_ts_bm), as.double(tail(ts_bm, 1)))
