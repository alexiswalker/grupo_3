...
```{r}
t <- diff(ts_ipc, lag = 1)
m <- arima(window(t), order = c(1,0,1))
f <- forecast::forecast(m, h = 6)
f

t <- ts_ipc
m <- arima(window(t), order = c(1,1,1))
f <- forecast(m, h = 6)
f
```

```{r}
nn_model <- ipc %>% rename(ds = fecha, y = indice_precios_consumidor_acumulada) %>% prophet
future <- make_future_dataframe(nn_model, periods = 6, include_history = F, freq = "month")
forecast <- predict(nn_model, future)
plot(nn_model, forecast)
