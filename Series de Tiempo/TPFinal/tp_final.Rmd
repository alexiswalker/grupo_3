...
Las primeras dos son consideradas fundamentales para el desarrollo de un país ya que conllevan, en la relación entre ambas, dos efectos antagónicos según se comporten. 

Por un lado, cómo evoluciona en nivel de vida de la población. Si los salarios se incrementan en mayor proporción que los bienes y servicios se mejora el bienestar ocasionando mayor demanda y, consecuentemente, el incremento de la producción.
...
Además, como resultado del actual proceso inflacionario (elemento que sugiere incluir también en el análisis la Base Monetaria) y de la baja demanda interna hay que mencionar el perjuicio que se ocasiona a los sectores productivos internos.
...
adjust_forecast <- function(forecast_object, ts_name, mean, lower, upper){

  forecast_object$forecast[[ts_name]]$mean = mean
  forecast_object$forecast[[ts_name]]$lower[, 1] = lower[, 1]
  forecast_object$forecast[[ts_name]]$lower[, 2] = lower[, 2]
  forecast_object$forecast[[ts_name]]$upper[, 1] = upper[, 1]
  forecast_object$forecast[[ts_name]]$upper[, 2] = upper[, 2]
...
La percepción individual sugiere que los motivos, en la coyuntura actual, son variados: especulación, emisión desmedida, presión por devaluación, bajas exportaciones, monopolios formadores de precios, elevado gasto público, mercado laboral altamente regulado, etcétera.
...
Así mismo, se agrega al estudio la Base Monetaria, atendiendo a la conjetura colectiva del estrecho vínculo que existe entre emisión e inflación (sin dejar de considerar la posibles causas enumeradas anteriormente de la primera variable). Ésto con intensión de reconocer si genera impacto en el Índice de Precios al Consumidor.

```{r ipc-vs-is-vs-bm, fig.cap="IPC vs IS vs BM", fig.height=4, fig.width=10}
autoplot(ts_ipc, series = 'Índice de Precios al Consumidor (IPC)') +
  autolayer(ts_is, series = 'Índice de Salarios (IS)') +
  autolayer(ts_bm, series = 'Base Monetaria (BM)') +
...
La cantidad de retardos que constituyen el modelo de mejor ajuste surge de calcular una serie de criterios de información para una secuencia de valores asignadas al parámetro. 

Uno de ellos es el Error de Predicción Final (FPE de sus siglas en inglés). Según @ljung, éste proporciona una medida de calidad al simular la situación en la que el modelo se prueba con un conjunto de datos diferente. Según la teoría de Akaike, el más preciso tiene el FPE más pequeño, que representa un equilibrio entre el número de parámetros y la variación explicada.

Para determinar el aporte que una variable realiza para la predicción de otra, @granger propone una prueba donde determinar si el valor actual de $X_t$ se "predice" mejor cuando el valor actual de $Y_t$ es incluido en el modelo.

## Prophet 

Según @prophet, esta técnica propone un modelo de serie de tiempo con tres componentes principales: tendencia, estacionalidad y vacaciones. Los cuales se combinan conforme la ecuación \@ref(eq:prophet-eq).
...
Donde $g(t)$ es la función de tendencia que modela cambios no periódicos, $s(t)$ representa cambios periódicos (por ejemplo, estacionalidad semanal y anual), y $h(t)$ representa los efectos de las vacaciones. El término de error $\epsilon_t$ representa cualquier cambio que no se adapte al modelo.

Se enmarca el problema de pronóstico como un ejercicio de ajuste de curvas, que es inherentemente diferente de los modelos de series de tiempo que explican la estructura de dependencia temporal en los datos. 

Esta formulación proporciona una serie de ventajas prácticas: flexibilidad, es posible acomodar fácilmente la estacionalidad con múltiples períodos y permitir que el analista haga diferentes suposiciones sobre las tendencias; no es necesario espaciar regularmente las medidas y no es necesario interpolar los valores faltantes; el ajuste es muy rápido, lo que permite al analista explorar de forma interactiva muchas especificaciones; y, por último, el modelo de pronóstico tiene parámetros fáciles de interpretar que se pueden cambiar para imponer suposiciones.
...
Mediante la función `ndiffs` de R, se estima que el numero de diferenciaciones necesarias para convertir a la serie Base Monetaria en estacionaria es `r bm_ndiffs`.
...
Con el objeto principal de pronosticar y analizar causalidad entre series se proponen modelos agrupando de a pares los datos que se utilizan.

Se calculan y muestran distintos criterios de selección para determinar el modelo que mejor ajusta, sin embargo se trabaja con el *Error de Predicción Final* (FPE) para determinar que retardo utilizar.
...
En la tabla \@ref(tab:var-selection-ipc-bm), se sugieren que el uso de retardo igual a `r lags` es apropiado para estimar el modelo. El resumen del mismo se incluye en el [Apendice](#ipc-bm-modelo-var).
...
En la figura \@ref(fig:pronostico-ipc-bm) se presenta el resultado gráfico de estimar las variables incluidas.

```{r pronostico-ipc-bm, fig.cap="Predicciones", fig.height=4, fig.width=10, warning=FALSE, fig.pos="H"}

ipc_bm_model_forecast <- forecast(ipc_bm_model, h = 9)

ipc_bm_model_forecast$forecast$train_ipc$mean = window(diff_inv(join_ts(train_ipc, ipc_bm_model_forecast$forecast$train_ipc$mean), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_bm_model_forecast$forecast$train_ipc$lower[,1] = window(diff_inv(join_ts(train_ipc, ipc_bm_model_forecast$forecast$train_ipc$lower[,1]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_bm_model_forecast$forecast$train_ipc$lower[,2] = window(diff_inv(join_ts(train_ipc, ipc_bm_model_forecast$forecast$train_ipc$lower[,2]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_bm_model_forecast$forecast$train_ipc$upper[,1] = window(diff_inv(join_ts(train_ipc, ipc_bm_model_forecast$forecast$train_ipc$upper[,1]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_bm_model_forecast$forecast$train_ipc$upper[,2] = window(diff_inv(join_ts(train_ipc, ipc_bm_model_forecast$forecast$train_ipc$upper[,2]), ts_ipc, ipc_ndiffs), start = start(test_ipc))

ipc_bm_model_forecast$forecast$train_bm$mean = window(diff_inv(join_ts(train_bm, ipc_bm_model_forecast$forecast$train_bm$mean), ts_bm, bm_ndiffs), start = start(test_bm))
ipc_bm_model_forecast$forecast$train_bm$lower[,1] = window(diff_inv(join_ts(train_bm, ipc_bm_model_forecast$forecast$train_bm$lower[,1]), ts_bm, bm_ndiffs), start = start(test_bm))
ipc_bm_model_forecast$forecast$train_bm$lower[,2] = window(diff_inv(join_ts(train_bm, ipc_bm_model_forecast$forecast$train_bm$lower[,2]), ts_bm, bm_ndiffs), start = start(test_bm))
ipc_bm_model_forecast$forecast$train_bm$upper[,1] = window(diff_inv(join_ts(train_bm, ipc_bm_model_forecast$forecast$train_bm$upper[,1]), ts_bm, bm_ndiffs), start = start(test_bm))
ipc_bm_model_forecast$forecast$train_bm$upper[,2] = window(diff_inv(join_ts(train_bm, ipc_bm_model_forecast$forecast$train_bm$upper[,2]), ts_bm, bm_ndiffs), start = start(test_bm))

autoplot(window(ts_ipc, end=end(train_ipc)),
         series = "Indice Precios Consumidor") +
  autolayer(ipc_bm_model_forecast$forecast$train_ipc,
            series = "Pronostico Indice Precios Consumidor",
            alpha = 0.5) +
  autolayer(window(ts_bm, end=end(train_bm)),
            series = "Base Monetaria") +
  autolayer(ipc_bm_model_forecast$forecast$train_bm,
...
```
```{r}
var_bm_pred <- head(ipc_bm_model_forecast$forecast$train_bm$mean, 3)
var_ipc_pred <- head(ipc_bm_model_forecast$forecast$train_ipc$mean, 3)
var_bm_real <- tail(ts_bm, 3)
var_ipc_real <- tail(ts_ipc, 3)
bm_var_ipc_bm_accuracy <- accuracy(var_bm_real, var_bm_pred)
ipc_var_ipc_bm_accuracy <- accuracy(var_ipc_real, var_ipc_pred)
```
...
En la tabla \@ref(tab:var-selection-ipc-is), se sugieren que el uso de retardo igual a `r lags` es apropiado para estimar el modelo. El resumen se incluye en el [Apendice](#ipc-is-modelo-var).
...
En la figura \@ref(fig:pronostico-ipc-is) se presenta el resultado gráfico de estimar las variables incluidas.

```{r pronostico-ipc-is, fig.cap="Predicciones", fig.height=4, fig.width=10, warning=FALSE, fig.pos="H"}

ipc_is_model_forecast <- forecast(ipc_is_model, h = 9)

ipc_is_model_forecast$forecast$train_ipc$mean = window(diff_inv(join_ts(train_ipc, ipc_is_model_forecast$forecast$train_ipc$mean), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_is_model_forecast$forecast$train_ipc$lower[,1] = window(diff_inv(join_ts(train_ipc, ipc_is_model_forecast$forecast$train_ipc$lower[,1]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_is_model_forecast$forecast$train_ipc$lower[,2] = window(diff_inv(join_ts(train_ipc, ipc_is_model_forecast$forecast$train_ipc$lower[,2]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_is_model_forecast$forecast$train_ipc$upper[,1] = window(diff_inv(join_ts(train_ipc, ipc_is_model_forecast$forecast$train_ipc$upper[,1]), ts_ipc, ipc_ndiffs), start = start(test_ipc))
ipc_is_model_forecast$forecast$train_ipc$upper[,2] = window(diff_inv(join_ts(train_ipc, ipc_is_model_forecast$forecast$train_ipc$upper[,2]), ts_ipc, ipc_ndiffs), start = start(test_ipc))

ipc_is_model_forecast$forecast$train_is$mean = window(diff_inv(join_ts(train_is, ipc_is_model_forecast$forecast$train_is$mean), ts_is, is_ndiffs), start = start(test_is))
ipc_is_model_forecast$forecast$train_is$lower[,1] = window(diff_inv(join_ts(train_is, ipc_is_model_forecast$forecast$train_is$lower[,1]), ts_is, is_ndiffs), start = start(test_is))
ipc_is_model_forecast$forecast$train_is$lower[,2] = window(diff_inv(join_ts(train_is, ipc_is_model_forecast$forecast$train_is$lower[,2]), ts_is, is_ndiffs), start = start(test_is))
ipc_is_model_forecast$forecast$train_is$upper[,1] = window(diff_inv(join_ts(train_is, ipc_is_model_forecast$forecast$train_is$upper[,1]), ts_is, is_ndiffs), start = start(test_is))
ipc_is_model_forecast$forecast$train_is$upper[,2] = window(diff_inv(join_ts(train_is, ipc_is_model_forecast$forecast$train_is$upper[,2]), ts_is, is_ndiffs), start = start(test_is))

autoplot(window(ts_ipc, end=end(train_ipc)),
         series = "Indice Precios Consumidor") +
  autolayer(ipc_is_model_forecast$forecast$train_ipc,
            series = "Pronostico Indice Precios Consumidor",
            alpha = 0.5) +
  autolayer(window(ts_is, end=end(train_is)),
            series = "Índice de Salarios") +
  autolayer(ipc_is_model_forecast$forecast$train_is,
            series = "Pronostico Índice de Salarios",
            alpha = 0.5) +
  labs(x = "Tiempo", y = "Porcentaje Acumulado") +
  ggtitle("Series con sus pronosticos")
```
...
```{r}
var_ipc_pred <- head(ipc_is_model_forecast$forecast$train_ipc$mean, 3)
var_is_pred <- head(ipc_is_model_forecast$forecast$train_is$mean, 3)
var_ipc_real <- tail(ts_ipc, 3)
var_is_real <- tail(ts_is, 3)
ipc_var_ipc_is_accuracy <- accuracy(var_ipc_real, var_ipc_pred)
is_var_ipc_is_accuracy <- accuracy(var_is_real, var_is_pred)
```
...
En la tabla \@ref(tab:var-selection-is-bm), se sugieren que el uso de retardo igual a `r lags` es apropiado para estimar el modelo. El resumen se incluye en el [Apendice](#is-bm-modelo-var).
...
En la figura \@ref(fig:pronostico-bm-is) se presenta el resultado gráfico de estimar las variables incluidas.

```{r pronostico-bm-is, fig.cap="Predicciones", fig.height=4, fig.width=10, warning=FALSE, fig.pos="H"}

is_bm_model_forecast <- forecast(is_bm_model, h = 9)

is_bm_model_forecast$forecast$train_is$mean = window(diff_inv(join_ts(train_is, is_bm_model_forecast$forecast$train_is$mean), ts_is, is_ndiffs), start = start(test_is))
is_bm_model_forecast$forecast$train_is$lower[,1] = window(diff_inv(join_ts(train_is, is_bm_model_forecast$forecast$train_is$lower[,1]), ts_is, is_ndiffs), start = start(test_is))
is_bm_model_forecast$forecast$train_is$lower[,2] = window(diff_inv(join_ts(train_is, is_bm_model_forecast$forecast$train_is$lower[,2]), ts_is, is_ndiffs), start = start(test_is))
is_bm_model_forecast$forecast$train_is$upper[,1] = window(diff_inv(join_ts(train_is, is_bm_model_forecast$forecast$train_is$upper[,1]), ts_is, is_ndiffs), start = start(test_is))
is_bm_model_forecast$forecast$train_is$upper[,2] = window(diff_inv(join_ts(train_is, is_bm_model_forecast$forecast$train_is$upper[,2]), ts_is, is_ndiffs), start = start(test_is))

is_bm_model_forecast$forecast$train_bm$mean = window(diff_inv(join_ts(train_bm, is_bm_model_forecast$forecast$train_bm$mean), ts_bm, bm_ndiffs), start = start(test_bm))
is_bm_model_forecast$forecast$train_bm$lower[,1] = window(diff_inv(join_ts(train_bm, is_bm_model_forecast$forecast$train_bm$lower[,1]), ts_bm, bm_ndiffs), start = start(test_bm))
is_bm_model_forecast$forecast$train_bm$lower[,2] = window(diff_inv(join_ts(train_bm, is_bm_model_forecast$forecast$train_bm$lower[,2]), ts_bm, bm_ndiffs), start = start(test_bm))
is_bm_model_forecast$forecast$train_bm$upper[,1] = window(diff_inv(join_ts(train_bm, is_bm_model_forecast$forecast$train_bm$upper[,1]), ts_bm, bm_ndiffs), start = start(test_bm))
is_bm_model_forecast$forecast$train_bm$upper[,2] = window(diff_inv(join_ts(train_bm, is_bm_model_forecast$forecast$train_bm$upper[,2]), ts_bm, bm_ndiffs), start = start(test_bm))

autoplot(window(ts_is, end=end(train_is)),
         series = "Índice de Salarios") +
  autolayer(is_bm_model_forecast$forecast$train_is,
            series = "Pronostico Índice de Salarios",
            alpha = 0.5) +
  autolayer(window(ts_bm, end=end(train_bm)),
            series = "Base Monetaria") +
  autolayer(is_bm_model_forecast$forecast$train_bm,
            series = "Pronostico Base Monetaria",
            alpha = 0.5) +
  labs(x = "Tiempo", y = "Porcentaje Acumulado") +
  ggtitle("Series con sus pronosticos")
...
```{r}
var_is_pred <- head(is_bm_model_forecast$forecast$train_is$mean, 3)
var_bm_pred <- head(is_bm_model_forecast$forecast$train_bm$mean, 3)
var_is_real <- tail(ts_is, 3)
var_bm_real <- tail(ts_bm, 3)
is_var_is_bm_accuracy <- accuracy(var_is_real, var_is_pred)
bm_var_is_bm_accuracy <- accuracy(var_bm_real, var_bm_pred)
```
...
Otra forma de modelar las series analizadas anteriormente es mediante Prophet. Se trabaja con cada una de forma independiente, separando el conjunto de datos en train y test. Donde se considera train hasta el 2021-04-01 y test hasta el 2021-07-01.

Una de las ventajas que posee Prophet es que puede tener en cuenta eventos como las vacaciones nacionales en el análisis de la serie. Debido a que gran parte de la economía argentina responde a situaciones políticas del momento, resulta interesante tener en cuenta los siguientes eventos:
...
Se utiliza esta técnica para modelar las 3 series, teniendo en cuenta dichos eventos.

```{r}
holidays <- read.csv("https://docs.google.com/uc?id=1UEoytRJY-R_MzbKiT11AvEmQWdfxC2ht")
```
...
Se entrena un modelo Prophet para Índice de Precios al Consumidor y se analizan las métricas de performance. En la Tabla \@ref(tab:ipc-prophet-acc) se detallan las métricas de testing obtenidas. 

```{r ipc-prophet-acc}
ipc_x_test <- make_future_dataframe(ipc_prophet_model, 
                                    periods = 3, 
                                    freq = "month", 
                                    include_history = FALSE)
ipc_y_pred <- predict(ipc_prophet_model, ipc_x_test)
ipc_prophet_accuracy <- accuracy(x = ipc_test$y, ipc_y_pred$yhat)

metrics_ipc_pp <- ipc_prophet_accuracy %>% 
  as_tibble() %>% 
  add_column(Modelo = "Prophet", .before = "ME") 

metrics_ipc_var_ipc_bm <- ipc_var_ipc_bm_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(ipc_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IPC-BM", .before = "ME") 

metrics_ipc_var_is_bm <- ipc_var_ipc_is_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(ipc_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IPC-IS", .before = "ME") 

rbind(metrics_ipc_pp, metrics_ipc_var_ipc_bm, metrics_ipc_var_is_bm) %>% 
  kable(booktabs = T, 
        caption = "Métricas de los modelos de Indice de Precios al Consumidor", 
        digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
```{r fig.cap="Predicciones de IPC", fig.height=4, fig.width=8}
plot(ipc_prophet_model, ipc_prophet_forecast) + 
  ggtitle("Índice de Precios al Consumidor", subtitle = "Con prediccion de 9 meses") +
  ylab("% cambio acumulado") +
  xlab("Fecha")
```

Ambos modelos $VAR$ que ajustan IPC obtuvieron mejores métricas en test a comparación del modelo $Prophet$. Sin embargo, los intervalos de confianza de las predicciones son mas acotados en el modelo $Prophet$ que los obtenidos con los modelos $VAR$.

La Figura \@ref(fig:ipc-pp-componentes) muestra la descomposición de los componentes del modelo.

```{r ipc-pp-componentes, fig.cap="Componentes del modelo Prophet", fig.height=4, fig.width=8}
prophet_plot_components(ipc_prophet_model, ipc_prophet_forecast)
```

Analizando las componentes, se concluye que:

- El Índice de Precios al Consumidor acumulado presenta una tendencia positiva, de forma cuasi lineal.
- Ambas elecciones afectaron la serie de forma positiva, aumentando entre 1,5pp y 3pp por sobre la tendencia. Del otro lado, el inicio del aislamiento por COVID afecta la serie disminuyendo un 1pp.
...
Se entrena un modelo Prophet para Índice de Salarios y se analizan las métricas de performance. En la Tabla \@ref(tab:is-prophet-acc) se detallan las métricas de testing obtenidas. 

```{r is-prophet-acc}
is_x_test <- make_future_dataframe(is_prophet_model, 
                                   periods = 3, 
                                   freq = "month", 
                                   include_history = FALSE)
is_y_pred <- predict(is_prophet_model, is_x_test)
is_prophet_accuracy <- accuracy(x = is_test$y, is_y_pred$yhat)

metrics_is_pp <- is_prophet_accuracy %>% 
  as_tibble() %>% 
  add_column(Modelo = "Prophet", .before = "ME") 

metrics_is_var_ipc_is <- is_var_ipc_is_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(is_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IPC-IS", .before = "ME") 

metrics_is_var_is_bm <- is_var_is_bm_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(is_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IS-BM", .before = "ME") 

rbind(metrics_is_pp, metrics_is_var_ipc_is, metrics_is_var_is_bm) %>% 
  kable(booktabs = T, 
        caption = "Métricas de los modelos de Indice de Salarios", 
        digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
```{r fig.cap="Predicciones de Indice Salarios", fig.height=4, fig.width=8}
...
Ambos modelos $VAR$ que modelan el Índice de Salarios obtuvieron mejores métricas en test a comparación del modelo $Prophet$. Sin embargo, los intervalos de confianza de las predicciones son mas acotados en el modelo $Prophet$ que los obtenidos con los modelos $VAR$.

La Figura \@ref(fig:is-pp-componentes) muestra la descomposición de los componentes del modelo.

```{r is-pp-componentes, fig.cap="Componentes del modelo Prophet", fig.height=4, fig.width=8}
prophet_plot_components(is_prophet_model, is_prophet_forecast)
```
Analizando las componentes, se concluye que:

- El Índice de Salarios acumulado presenta una tendencia positiva, de forma cuasi lineal.
- Ambas elecciones afectaron la serie de forma positiva, aumentando aproximadamente en un 2pp por sobre la tendencia. El inicio del aislamiento por COVID afecta la serie aumentándola un 3,5pp.
- La serie presenta una estacionalidad positiva en diciembre, aumentando el valor de la misma para mediados de mes.
...
Se entrena un modelo Prophet para la Base Monetaria acumulada y se analizan las métricas de performance. En la Tabla \@ref(tab:bm-prophet-acc) se detallan las métricas de testing obtenidas. 
```{r bm-prophet-acc}
bm_x_test <- make_future_dataframe(bm_prophet_model, 
                                   periods = 3, 
                                   freq = "month", 
                                   include_history = FALSE)
bm_y_pred <- predict(bm_prophet_model, bm_x_test)
bm_prophet_accuracy <- accuracy(x = bm_test$y, bm_y_pred$yhat)

metrics_bm_pp <- bm_prophet_accuracy %>% 
  as_tibble() %>% 
  add_column(Modelo = "Prophet", .before = "ME") 

metrics_bm_var_ipc_bm <- bm_var_ipc_bm_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(bm_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IPC-BM", .before = "ME") 

metrics_bm_var_is_bm <- bm_var_is_bm_accuracy %>% 
  as_tibble() %>% 
  dplyr::select(colnames(bm_prophet_accuracy)) %>% 
  add_column(Modelo = "VAR IS-BM", .before = "ME") 

rbind(metrics_bm_pp, metrics_bm_var_ipc_bm, metrics_bm_var_is_bm) %>% 
  kable(booktabs = T, 
        caption = "Métricas de los modelos de Base Monetaria", 
        digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
```{r fig.cap="Predicciones de Base Monetaria", fig.height=4, fig.width=8}
...
El modelo $Prophet$ es el que mejor ajusta a la serie de Base Monetaria, superando a los modelos $VAR$. A su vez, los intervalos de confianza de sus predicciones son mas acotadas que las de los modelos $VAR$.

La Figura \@ref(fig:bm-pp-components) muestra la descomposición de los componentes del modelo.

```{r bm-pp-components, fig.cap="Componentes del modelo Prophet", fig.height=4, fig.width=8}
prophet_plot_components(bm_prophet_model, bm_prophet_forecast)
```

Analizando las componentes, se concluye que:

- La Base Monetaria acumulada presenta una tendencia positiva, de forma cuasi lineal.
- Ambas elecciones afectaron la serie de forma negativa, disminuyendo aproximadamente en un 25pp la Base Monetaria acumulada por sobre la tendencia. El inicio del aislamiento por COVID afecta la serie aumentándola hasta un 20pp sobre la tendencia.
- A mitad de año la Base Monetaria presenta su pico máximo de estacionalidad positiva, coincidiendo con los aguinaldos.

\newpage

# Conclusiones

Queda en evidencia que la tendencia del Índice de Precios al Consumidor y el Índice de Salarios profundizan una brecha que son altamente negativas para la economía local.
...
Respecto a la causalidad o impacto que estos indicadores tienes entre si, hay falta de indicios en los modelos simplificados que la tendencia de la Base Monetaria afecte el Índice de Precios al Consumidor; la conjetura del efecto directo de la emisión en la inflación no se hace visible entre los índices utilizados. Parece que encontrar esta dependencia implica complejizar el análisis sumando información al estudio.

\begin{figure}
\centering
\begin{tikzpicture}[node distance={35mm}, thick, main/.style = {draw, rectangle}] 
\node[main] (1) {Índice de Precios al Consumidor}; 
\node[main] (2) [below left of=1] {Índice de Salarios}; 
\node[main] (3) [below right of=1] {Base Monetaria}; 
\draw[->] (2) -- (1); 
\draw[->] (2) -- (3); 
\draw[->] (1) -- (3);
\end{tikzpicture} 
...
Por otro lado, existen eventos de carácter político que dejan huella en las lecturas obtenidas en torno al momento de su ocurrencia; conjuntos de medidas gubernamentales que tienen impacto en las series analizadas. Particularmente, la calidad del ajuste sugiere que las elecciones y la pandemia generaron una alteración del comportamiento, especialmente visible en la Base Monetaria. 

Desde un punto de vista técnico, las series que tiene una variación relativamente constante en signo y valor (IPC e Índice de Salarios), son mejor explicadas con la incorporación de las lecturas de otras. Mientras la que muestra rango mensuales más amplios y en distintas direcciones (Base Monetaria), la segunda técnica aplicada ajusta mejor; algo visible no solo en los criterios de información sino también en los amplios intervalos de confianza producto de su variabilidad.
