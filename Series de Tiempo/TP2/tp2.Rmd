...
La combinación de los modelos $ARMA(p, d)$ estacionales y no estacionales se expresa como: $ARMA(p, q)(P, Q)s$, donde $P$ es el componente autorregresivo estacional, $Q$ el componente de media móvil estacional y $s$ representa el período estacional.
...
Se procede a analizar la estacionariedad de la serie mediante los tests de Dickey Fuller (ADF) y  Kwiatkowski–Phillips–Schmidt–Shin (KPSS).
...
@kpss definen otra prueba en la cual la hipótesis nula determina que una serie temporal observable es estacionaria. Los resultados de la misma se encuentran detallados en la Tabla \@ref(tab:ipc-kpss).
...
En conclusión, ambos tests concluyen que la serie de IPC es _no estacionaria_.
...
```{r ipc-diferenciado, fig.cap="Índice de precio al consumidor diferenciado", fig.height=5, fig.width=10, fig.pos="H"}
...
TODO: residuos que claramente no pasarian el test de ADF.

\newpage

## Índice de Salarios

La Figura \@ref(tab:is) muestra la serie de Índice de Salarios.
...
```
Se procede a analizar la estacionariedad de la misma.
...
El test KPSS sobre la serie diferenciada concuerda con el test ADF, cocluyendo que la serie diferenciada es _estacionaria_.
