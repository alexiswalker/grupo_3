...
P es el componente autorregresivo estacional, Q el componente de media móvil estacional y s representa el período estacional.

\begin{equation}
  \Theta_{P} (B^{s}) \phi_{p}(B)Y_{t} = \Theta_{Q} (B^{s}) \theta_{q}(B)\epsilon_{t}
\end{equation}

El presente trabajo práctico tiene como objetivo el análisis y predicción las series descriptas anteriormente mediante modelos ARMA estacionales y no estacionales.
...
```{r}
shapiro_wilk_normality_test(residuos_sarima_ipc) %>% 
  kable(booktabs = T, caption = "Prueba Shapiro-Wilk de normalidad de los residuos", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
