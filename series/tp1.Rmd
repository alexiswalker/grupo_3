...
El modelo que resultó del proceso iterativo propuesto por el auto-ml es superador en los criterios AIC y BIC pero ligeramente peor en las medidas ME y RSME. Se opta por seleccionar el SARIMA debido a sus métricas de informacion. Sin embargo, de ser mas estrictos, se podría desempatar realizando cross-validation en el entrenamiento de los modelos (@rafferty, cap. 11).
...
```{r}
bind_cols(
  window(ts_indice_precios_consumidor, start = c(2021, 5)) %>% as_tsibble()  %>% rename(real = value, mes = index),
  forecast(model_arima_ma_ipc, h = 3) %>% as_tibble() %>% select('Point Forecast') %>% rename(estimado = 'Point Forecast')
  ) %>% 
...
```{r}
bind_cols(
  window(ts_indice_salarios, start = c(2021, 5)) %>% as_tsibble()  %>% rename(real = value, mes = index),
  forecast(model_arima_auto_is, h = 3) %>% as_tibble() %>% select('Point Forecast') %>% rename(estimado = 'Point Forecast')
  ) %>% 
