...
```{r}
auto.arima(ts_indice_precios_consumidor)
```
...
ndiffs(ts_indice_precios_consumidor, max.d = 4, test="kpss", type="trend")

summary(ur.df(ts_indice_precios_consumidor, type = "none", lags = 6, selectlags = "AIC"))
...
TODO: esto lo dejamos? deberíamos preguntar en el foro:
...
```{r}
dif_ts_indice_precios_consumidor

adf.test(dif_ts_indice_precios_consumidor)
kpss.test(ts_indice_precios_consumidor, null = "Trend")
```
...
aic_data_indice_precios_consumidor <- aic_grid_search(dif_ts_indice_precios_consumidor, 4, 4)
...
bic_data_indice_precios_consumidor <- bic_grid_search(dif_ts_indice_precios_consumidor, 4, 4)
...
```{r}
model_6_1 <- arima(train_data_ipc, order = c(6,0,1), include.mean = F, fixed=c(0,0,0,0,0,NA,NA), transform.pars = FALSE, method="ML")
my_forecast_6_1 <- forecast(model_6_1, h = length(test_data_ipc))
accuracy_6_1 <- accuracy(test_data_ipc, my_forecast_6_1$mean)

metrics_6_1 <- accuracy_6_1 %>%
  as_tibble() %>%
  add_column(Modelo = "seis", .before = "ME")
...
```{r}
auto_model_ipc <- auto.arima(train_data_ipc, seasonal = T)
...
rbind(metrics_aic_bic_ipc, metrics_auto_ipc, metrics_6_1) %>%
  kable(booktabs = T, caption = "Métricas", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 7)
```
...
ggtsdisplay(residuals(model_6_1), lag.max = 24, main = 'Residuos', plot.type = "histogram")
```
...
colors <- c("train" = "blue", "test" = "red", "forecast" = "orange", "auto" = "gray", "seis" = "black")
ggplot() +
  ...
  geom_line(data = as_tsibble(my_forecast_6_1$mean), aes(x = index, y = value, color = "seis")) +
...
```{r}
model_aic_is <- arima(train_data_is, order = c(2,0,6), include.mean = F)
