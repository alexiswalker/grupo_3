...
El lag numero 6 en la funcion de autocorrelacion es significativo, por lo que se puede proponer un MA(6) para modelarlo. TODO: agregar alguna referencia donde diga que en ACF modele el parametro del MA
...
TODO: decir que separamos en train / test hasta abril
...
```{r}
model_aic_bic_ipc <- arima(train_data_ipc, 
                           order = c(1,0,1), 
                           include.mean = F)
summary(model_aic_bic_ipc)
```
...
Se resumen las métricas de los modelos propuestos (Tabla 3).
...
Por Parsimonia, se selecciona el modelo ARMA(0,6) ya que sus $\theta_i=0$ cuando $i=1,2,3,4,5$, y $\phi_6=0.3102$.

#### Análisis de Diagnóstico

```{r fig.cap="Residuos", fig.height=3.5, fig.width=10, warning=FALSE, fig.pos="H"}
ggtsdisplay(residuals(model_ma_ipc), 
            lag.max = 24, 
            main = 'Residuos ARMA(0,6)', 
            plot.type = "histogram")
```
...
```{r}
test_incorrelation(residuals(model_ma_ipc))
```
...
Aun asi, ambos tests coinciden que no existe correlacion en los residuos del modelo.

La normalidad de los errores se corrobora a partir del test de Shapiro-Wilk con un p-value `r shapiro.test(residuals(model_ma_ipc))[["p.value"]]`. TODO: explicar shapiro

#### Pronóstico

Se comparan los modelos analizados en una prueba de pronóstico (Fig. 11).

```{r fig.cap="Predicciones", fig.height=3.5, fig.width=10, warning=FALSE, fig.pos="H"}
model_arima_ma_ipc <- arima(window(ts_indice_precios_consumidor, end = c(2021, 4)), 
                            order = c(0, 2, 6), 
                            fixed = c(0, 0, 0, 0, 0, NA), 
                            include.mean = F)

autoplot(forecast(model_arima_ma_ipc, h = 9), ylab = 'Porcentaje Acumulado')
```
