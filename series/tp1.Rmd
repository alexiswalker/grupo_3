...
```{r warning=FALSE, fig.cap="Criterios de Información", fig.height=4, fig.width=10}

aic_data <- aic_grid_search(dif_ts_indice_precios_consumidor, 5, 5)

aic_plot <- ggplot(aic_data, aes(p, q)) +
...
bic_plot <- ggplot(bic_data, aes(p, q)) +
...
```{r}
metrics_aic_bic <- accuracy_aic_bic %>%
  as_tibble() %>%
...
```{r}
metrics_auto <- acurracy_auto %>%
  as_tibble() %>%
  add_column(Modelo = "ARIMA(1,0,1)", .before = "ME")

rbind(metrics_aic_bic, metrics_auto) %>%
  kable(booktabs = T, caption = "Métricas", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
```{r fig.cap="Residuos", fig.height=5, fig.width=10, warning=FALSE}
residuals <- resid(model_aic_bic)
checkresiduals(residuals)
```

#### Pronóstico

```{r fig.cap="Pronóstico", fig.height=4, fig.width=10}
...
```{r}
metrics_aic <- accuracy_aic %>%
  as_tibble() %>%
  add_column(Modelo = "arima(order = c(1, 0, 2))", .before = "ME")
...
```{r}
metrics_bic <- accuracy_bic %>%
  as_tibble() %>%
  add_column(Modelo = "arima(order = c(1, 0, 0))", .before = "ME")
...
metrics_auto <- acurracy_auto %>%
  as_tibble() %>%
  add_column(Modelo = "ARIMA(1,0,0)(1,0,0)[12]", .before = "ME")

rbind(metrics_aic, metrics_bic, metrics_auto) %>%
  kable(booktabs = T, caption = "Métricas", digits = 3) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
