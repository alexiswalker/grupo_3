...
```{r message=FALSE}
paquetes <- c("readr", "tidyverse", "gridExtra", "kableExtra", "tseries", "xts",
              "forecast", "ggfortify", "lessR", "fable", "tsibble", "tibbletime")
...
indice_precios_consumidor <- raw_data %>% 
  select(fecha, indice_precios_consumidor)

indice_salarios <- raw_data %>%
  select(fecha, indice_salarios)

ts_indice_precios_consumidor <- indice_precios_consumidor$indice_precios_consumidor %>%
  ts(frequency = 12, start = c(2017,01))

ts_indice_salarios <- indice_salarios$indice_salarios %>%
  ts(frequency = 12, start = c(2017,01))
```
...
ggplot(raw_data, aes(x = fecha)) +
  geom_line(aes(y = indice_precios_consumidor), color = "red3") +
  geom_line(aes(y = indice_salarios), color = "steelblue") +
  scale_y_continuous(
...
```{r fig.cap="Índice de precio al consumidor y su tendencia", fig.height=4, fig.width=10}
dif_ts_indice_precios_consumidor <- ts_indice_precios_consumidor %>% diff(1)

dif_indice_precios_consumidor <- data.frame(
  indice_precios_consumidor = as.vector(dif_ts_indice_precios_consumidor),
  fecha = indice_precios_consumidor$fecha[-1])
...
```{r}
adf<-adf.test(dif_ts_indice_salarios)
adf$p.value < 5/100

adf<-adf.test(dif_ts_indice_precios_consumidor)
adf$p.value < 5/100

kpss<-kpss.test(dif_ts_indice_salarios)
kpss$p.value < 5/100

kpss<-kpss.test(dif_ts_indice_precios_consumidor)
kpss$p.value < 5/100
...
decompose_plot <- dif_ts_indice_precios_consumidor %>% decompose(type = 'additive') %>% autoplot(ts.colour = 'steelblue')
...
```{r fig.cap="Funciones", fig.height=5, fig.width=10, warning=FALSE}
decompose_plot <- dif_ts_indice_salarios %>% decompose(type = 'additive') %>% autoplot(ts.colour = 'steelblue')
decompose_plot + xlab("Tiempo") + ggtitle("Descoposición aditiva")
```
...
acf_plot <- ggAcf(dif_ts_indice_precios_consumidor) +
  ggtitle("Función de autocorrelación") +
  ylab(label = "Autocorrelación")

pacf_plot <- ggAcf(dif_ts_indice_precios_consumidor, type = "partial") +
  ggtitle("Función de autocorrelación parcial") +
  ylab(label = "Autocorrelación parcial")

grid.arrange(acf_plot, pacf_plot, nrow = 2)


acf(ts_indice_salarios, type="covariance", plot=T)
```
...
acf_plot <- ggAcf(dif_ts_indice_salarios) +
  ggtitle("Función de autocorrelación") +
  ylab(label = "Autocorrelación")

pacf_plot <- ggAcf(dif_ts_indice_salarios, type = "partial") +
  ggtitle("Función de autocorrelación parcial") +
  ylab(label = "Autocorrelación parcial")
...
```{r}
modelo <- arma(ts_indice_precios_consumidor, order = c(1, 0), include.intercept = T)
...
```{r}
train_data <- ts_indice_precios_consumidor %>%
  as_tsibble() %>% 
  filter_index(~ '2020-12')

test_data <- ts_indice_precios_consumidor %>%
  as_tsibble() %>% 
...
forecast_data <- train_data %>% 
  model(arima = ARIMA(value), arima210 = ARIMA(value ~ pdq(2,2,0))) %>%
  forecast(h = 7) %>%
