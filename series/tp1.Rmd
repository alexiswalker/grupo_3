...
```{r fig.cap="Índice de precios al consumidor", fig.height=3, fig.width=9, fig.pos="H"}
...
```{r fig.cap="Índice de precio al consumidor y su tendencia", fig.height=3, fig.width=9, fig.pos="H"}
...
```{r fig.cap="Índice de Salarios", fig.height=3, fig.width=9, fig.pos="H"}
...
```{r fig.cap="Índice de Salarios y su tendencia", fig.height=3, fig.width=9, fig.pos="H"}
...
```{r warning=FALSE, fig.cap="Criterios de Información", fig.height=4, fig.width=10, fig.pos="H"}
...
En ambos casos, el modelo sugerido es ARMA(1, 1).
...
Se resumen las métricas del modelo propuesto.
...
Ambos análisis conducen al mismo resultado.
...
```{r fig.cap="Residuos", fig.height=5, fig.width=10, warning=FALSE, fig.pos="H"}
ggtsdisplay(residuals(auto_model_ipc), lag.max = 24, main = 'Residuos', plot.type = "histogram")
```

#### Pronóstico

Se comparan los modelos analizados en una prueba de pronóstico.
...
Se resumen las métricas para ambos modelos.
...
Nuevamente se aprovecha la posibilidad de determinar automaticamente un buen modelo y se suma el resultado a lo ya planteado.
