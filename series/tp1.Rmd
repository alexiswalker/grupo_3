...
Ambos indicadores son considerados fundamentales para el desarrollo de un país ya que conllevan en la relación entre ambos. Por un lado como evoluciona en nivel de vida de la población, si es que los salarios se incrementan en mayor proporción que los bienes y servicios, con la consiguiente mejora en el bienestar además del incremento de la producción de bienes y servicios que ocasiona la mayor demanda, o, como se demostrará en el desarrollo del presente, los indicadores de incremento de bienes y servicios crecen a mayor ritmo que los salarios, disminuyendo el bienestar de la masa asalariada.

Como resultado de este proceso inflacionario y de baja demanda interna hay que mencionar que esto perjudica a los sectores productores de bienes y servicios internos.
...
\end{align}
El test de Box-Pierce utiliza a $\sigma^2(\hat{\rho_j}(Y))\cong1/n$ como estimador de la varianza, lo que produce un sesgo al no rechazo de la hipótesis nula que los $\rho_1(Y)=\rho_2(Y)=...=\rho_j(Y)=0$. #TODO: agregar referencia?. El test de Ljung-Box utiliza otra estimación de la varianza para corregir el sesgo.
...
Se comparan los modelos analizados en una prueba de pronóstico (Figura \@ref(fig:ipc-pronostico)).
...
Tanto la función de autocorrelación (ACF) com la de autocorrelación parcial (PACF) presentan un lag significativo en 1, por lo que el proceso puede ser
modelado con un $ARMA(1,1)$.

### Modelado

Se procede a modelar los datos separándolos en train y test (de igual manera que la serie anterior): desde el inicio hasta abril del 2021 para entrenamiento y el resto (mayo del 2021 hasta julio del 2021) para pruebas.

Se generan dos modelos y se comparan para elegir el mejor según criterios de información.

El primero, según lo mencionado, es un $ARMA(1,1)$ (ver detalle del modelo en [Apéndice](#ap-is-modelo-arma)).

El segundo es un modelo $AR(1)$  estacionario con un período igual a 12, obtenido con la función auto.arima del paquete forecast en R (ver detalle del modelo en [Apéndice](#ap-is-modelo-auto-arima)).
...
El modelo que resutltó del proceso iterativo propuesto por la función auto.arima tiene se comporta con índice superiores. Por lo que el modelo $AR(1)$  estacionario con un período igual a 12 es el elegido para realizar el pronósito (en la medida que sus residuos se comportes según supuestos).
